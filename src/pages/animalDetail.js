...
<div class="container mx-auto my-8 px-4">
    <div class="max-w-6xl mx-auto bg-white p-8 shadow-card-lg rounded-2xl">
        ...
        <div class="flex justify-end space-x-4 pr-4">
            <button title="Hayvan Düzenleme sayfasına yönlendirir" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-6 rounded-lg shadow-md hover:shadow-lg transform hover:scale-105 transition duration-200 ease-in-out" id="editButton">
            ...
            <button title="Hayvanı veritabanından siler" class="bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-6 rounded-lg shadow-md hover:shadow-lg transform hover:scale-105 transition duration-200 ease-in-out" id="deleteButton">
                <span class="flex items-center space-x-2">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <!-- Trash Modal -->
    <div id="trashModal" class="hidden fixed inset-0 bg-white/30 backdrop-blur-sm flex items-center justify-center z-50">
...
function showDatas(allDatas) {
    ...
    const trashModalPanel = document.getElementById("trashModalPanel");
    const cancelTrashBtn = document.getElementById("cancelTrashBtn");
    const confirmTrashBtn = document.getElementById("confirmTrashBtn");
    ...
    deleteButton.addEventListener("click", () => {
        ...
        document.getElementById("confirmTrashBtn").addEventListener("click", () => {
            ...
            // window.electronAPI.moveToTrash(datas);
            window.electronAPI.removeAnimal(datas);
            window.close();
        });
    });
    ...
    if (allDatas.animalData[0].Type === "cow") {
        ...
        if (allDatas.calvesData.length != 0) {
            allDatas.calvesData.forEach((calf) => {
                ...
                if (calf.DeathDate) {
                    element.className =
                        "bg-red-200 rounded-lg p-4 border border-gray-200 hover:shadow-md transition duration-200";
                    ...
                    element.innerHTML = `
                        <div class="space-y-2">
                            ...
                            <p class="text-sm"><span class="font-bold text-gray-800">Sürüden Ayrılma:</span> <span class="text-gray-950">${new Date(
                                calf.DeathDate
                            ).toLocaleDateString("tr-TR")}</span></p>
                            <p class="text-sm"><span class="font-bold text-gray-800">Neden:</span> <span class="text-gray-950">
                                ${calf.Reason}
                            </span></p>
                        </div>
                    `;
        ...
        animalInfoCard.innerHTML += `
            ...
            allDatas.cowData[0].InseminationDate
                    ...
                    <hr class="border-gray-300">
                    <p><span class="font-bold text-gray-800">Not:</span> <span class="text-gray-800 font-medium">${allDatas.animalData[0].Note ?? ""}</span></p>
                </div>
            </div>
        `;
                ...
                if (calf.DeathDate) {
                    ...
                    element.innerHTML = `
                        <div class="space-y-2">
                            ...
                            <p class="text-sm"><span class="font-bold text-gray-800">Ölüm:</span> <span class="text-gray-950">${new Date(
                                calf.DeathDate
                            ).toLocaleDateString("tr-TR")}</span></p>
                            <p class="text-sm"><span class="font-bold text-gray-800">Neden:</span> <span class="text-gray-950">
                                ${calf.Reason}
